...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onNewTag = () => {
    ...
    // El push es una de las instrucciones que mutan entonces debería cambiar el estado
    currentTags.push(newTag);

    // setValue('tags', [...currentTags, newTagValue.trim()], {
    //   shouldValidate: true,
    // });
  };
  ...
  const onSubmitForm = async (formData: FormData) => {
    ...
    try {
      const { data } = await tesloApi({
        url: '/admin/products',
        method: formData._id ? 'PUT' : 'POST', //si tenemos un _id, entonce actualizar sino crear
        data: formData,
      });
      console.log({ data });
      if (!formData._id) {
